...
import com.example.moviezone.utils.FavoritesDiffUtil

class FavoritesAdapter: RecyclerView.Adapter<FavoritesAdapter.ViewHolder>() {

    private lateinit var binding: MovieItemBinding
    private var viewModelInteractor: HomeViewModelInteractor? = null
    private var favorites: List<FavoriteMovie> = listOf()

    fun setViewModelInteractor(viewModelInteractor: HomeViewModelInteractor?) {
        this.viewModelInteractor = viewModelInteractor
    }

    fun setFavorites(favorites: List<FavoriteMovie>) {
        val diffUtil = FavoritesDiffUtil(this.favorites, favorites)
    ...
    override fun onBindViewHolder(holder: FavoritesAdapter.ViewHolder, position: Int) {
        Glide.with(holder.itemView.context)
            .load(Const.TMDB_IMAGE_URL + favorites[position].posterPath)
        ...
        holder.rating.text = favorites[position].voteAverage.toString()

        holder.itemView.setOnClickListener {
            viewModelInteractor?.showMovieDetails(favorites[position].id, Const.HOME_PAGE_INDEX)
        }
    }
